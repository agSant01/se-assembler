...
public class OperationCodes
{
    ...
    {
        //int[] bits = { };
        if( ! this.code_lookup_table.ContainsKey(token))
        {
        ...
        return (int)this.code_lookup_table[token];
    }
    ...
    private void setup_register_table()
    {
        ...
        int i = 0;
        foreach(string r in this.tokens.register_tokens() )
        {
            this.register_table.Add(r, i);
            i += 1;
        }
    }
    ...
    private void setup_code_table()
    {
        ...
        foreach(string token in tokens.keywords())
        {
            this.code_lookup_table.Add(token, op_code);
            op_code += 1;
        }
